...
impl<'a> YamlParser<'a> {
    ...
    fn visit_stripped_pattern(&mut self, pattern_hash: &yaml::Hash) -> ParserResult<Pattern> {
    ...
    fn visit_material(&mut self, material: &yaml::Yaml) -> ParserResult<Material> {
        ...
        if material_type.as_ref() == "phong" {
            ...
            if material_hash.contains_key(key!("pattern")) {
                ...
                self.path.push(Segment::Key("patter".into()));
            ...
            if material_hash.contains_key(key!("reflectiveness")) {
                ...
                phong_material = phong_material.with_reflective(material_reflectiveness);
            }
...
"##;
        ...
        expected_cameras.insert(
            String::from("output1"),
            Camera::new(800, 600, 0.785).view_transform(
...
"##;
        ...
        expected_cameras.insert(
            String::from("output2"),
            Camera::new(1920, 1080, 1.047).view_transform(
...
"##;
        ...
        expected_cameras.insert(
            ...
            Camera::new(3840, 2160, 1.047).view_transform(
...
"##;
        ...
        let actual = result.unwrap_err();
        let expected = anyhow!(
            "Unknown light type 'bright_but_unknown_light' found at .document[0].item[0].light"
        );
        assert_eq!(actual.to_string(), expected.to_string());
    }
